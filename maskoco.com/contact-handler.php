<?php
/**
 * MASKO — contact / enquiry form handler
 * ---------------------------------------------------------------------------
 * Receives POSTs from the contact form (contact-us.html), validates them, and
 * emails the enquiry to the business inbox via authenticated SMTP (PHPMailer).
 *
 * Anti-spam measures baked in:
 *   • Authenticated SMTP with an aligned From  (SPF/DKIM friendly)
 ...
 *
 * NO client mailbox password is required: you authenticate as a sending
 * account you control (a Gmail App Password, or an on-domain mailbox), and the
 * enquiry is still delivered to info@maskoco.com (set as MAIL_TO in config.php).
 *
 * Requirements: PHP 8.0+, PHPMailer (bundled in ./vendor), and a filled-in
...
if (!is_file($autoload) || !is_file($configPath)) {
    respond(500, false, 'The mail service is not configured yet. Please email info@maskoco.com directly.');
}
require $autoload;
...
try {
    ...
    // Aligned sender keeps SPF/DKIM happy (stays out of spam).
    ...
    $mail->Body = <<<HTML
        <div style="font-family:Arial,Helvetica,sans-serif;font-size:15px;color:#222;line-height:1.6">
          <h2 style="margin:0 0 12px;color:#2b7de0">New website enquiry</h2>
    ...
    error_log('MASKO contact form mail error: ' . $mail->ErrorInfo);
    respond(500, false, 'Sorry, your message could not be sent right now. Please email info@maskoco.com or call +966 51 152 2501.');
}
